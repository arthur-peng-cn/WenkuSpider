...
def get_next_pages(sess, docid, start):
    timestamp = int(time.time()* 1000)
    api = f'https://wenku.baidu.com/ndocview/readerinfo?docId={docid}&type=html&clientType=1&pn={start}&t={timestamp}&isFromBdSearch=0&srcRef=https:%2F%2Fwenku.baidu.com%2Fview%2F{docid}&rn=50&powerId=2&bizName=mainPc'
    try:
        resp = sess.get(api)
        data = resp.json()
        htmlURLs= data.get('data', {}).get('htmlUrls', [])
        return htmlURLs
    except Exception as e:
        print(e)
        return []

def get_all_pages_url(sess, docid, pagenums):
    timestamp = int(time.time()* 1000)
    start = 3
    pages = get_next_pages(sess, docid, start)
    while start + 50 <= pagenums:
       start += 50
       ret = get_next_pages(sess, docid, start)
       if len(ret) == 0:
           break
       pages.extend(ret)
    return pages
...
def main():
    ...
    if len(urls) == 0:
        urls = ['https://wenku.baidu.com/view/9b4e90350b4c2e3f57276351\?aggId\=02906c9fd5bbfd0a795673b6\&fr\=catalogMain_text_ernie_recall_v1:wk_recommend_main1']
        urls = ['https://wenku.baidu.com/view/67aceed526fff705cc170a97?aggId=02906c9fd5bbfd0a795673b6&fr=catalogMain_text_ernie_recall_backup_new:wk_recommend_main1']
    ...
    for url in urls:
        ...
        try:
            ...
            doc_id= data.get('readerInfo', {}).get('docId', '')
        ...
        # PPT
        if data['readerInfo']['tplKey'] == 'new_view' and filetype in  ['ppt', 'pdf']:
            ...
            if data['readerInfo']['page'] > len(imgs):
                ...
                ext_images = get_all_pages_url(sess, doc_id, data['readerInfo']['page'])
                imgs.extend(ext_images)
            ...
            for i in range(len(pagenums)):
                ...
                req = sess.get(imgs[pagenums[i] - 1], headers=headers)
        ...
        # PDF WORD (EXCEL)
            ...
            for i in range(len(pagenums)):
                ...
                if not fonts_csss.get(pagenums[i]):
                    continue
                req = sess.get(fonts_csss[pagenums[i]], headers=headers)
                # status not 200?
            ...
            for i in range(len(pagenums)):
                ...
                if not jsons.get(pagenums[i]):
                    continue
                req = sess.get(jsons[pagenums[i]], headers=headers)
                # status not 200?
            ...
            for i in range(len(pagenums)):
                ...
                if not pngs.get(pagenums[i]):
                    continue
                req = sess.get(pngs[pagenums[i]], headers=headers)
                # status not 200?
